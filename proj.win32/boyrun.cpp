    ...
    int       nCmdShow)
{
    ...
    return BoyrunPlayer::createAndRun();
}

int BoyrunPlayer::createAndRun(void)
{
    BoyrunPlayer *host = BoyrunPlayer::sharedInstance();
    int ret = host->run();
    BoyrunPlayer::purgeSharedInstance();
    return ret;
}

BoyrunPlayer *BoyrunPlayer::s_sharedInstance = NULL;

BoyrunPlayer *BoyrunPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new BoyrunPlayer();
    }
    return s_sharedInstance;
}

void BoyrunPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
BoyrunPlayer::BoyrunPlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int BoyrunPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "boyrun-player");
...
void BoyrunPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void BoyrunPlayer::createViewMenu(void)
{
...
void BoyrunPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void BoyrunPlayer::updateWindowTitle(void)
{
}

void BoyrunPlayer::relaunch(void)
{
...
void BoyrunPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void BoyrunPlayer::onFileOpenProject(void)
{
...
void BoyrunPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"BoyrunPlayer");
...
void BoyrunPlayer::onFileProjectConfig(void)
{
...
void BoyrunPlayer::onFileRelaunch(void)
{
    relaunch();
}

void BoyrunPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void BoyrunPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void BoyrunPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void BoyrunPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void BoyrunPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT BoyrunPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    BoyrunPlayer *host = BoyrunPlayer::sharedInstance();
...
INT_PTR CALLBACK BoyrunPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
